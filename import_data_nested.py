...
This code provides the main process code to open a .DAT file from an E-Tracker logger and read the contents into 
a pandas data frame where I manipulate the data frame to provide a resulting file output that mimics that
which the now obsolete Deltrax software used to provide. The file output is in Excel just like the Deltrax output.
this code can handle a .DAT file which  has three phase power date recorded on it. I have a separate one for 
dealing with single phase content"""
...
def my_test_fun():
    def get_file_path():
        ...
        file_path = filedialog.askopenfilename(title="Select a .DAT File from your USB",
                                               filetypes=[("CSV Files", "*.DAT")])
        return file_path
    ...
    def save_file(df):
        ...
        exit(1)  # added to ensure this function closes after operation in tkinter when app window is closed.
    ...
    # now we create a base date to represent the unique base date used by original software to produce a
    # timedelta day count
    ...
    # We have to use the base value in a calc with the previous timedelta value to arrive at a readable
    # logged date value.
    # The 'date' column value in the df now mimics how it appears in the Deltrax report for use in our own
    # Excel or other
    ...
    # now we apply corrections for scaling column values to reflect odd raw values which seem to be adjusted
    # inside Deltrax
    ...
    # Now we have to do an internal calculation sequence to get a "KVA" value to mimic what appears in
    # the Deltrax report
    step1 = df['V1'] * df['I1']
    step2 = df['V2'] * df['I2']
    step3 = df['V3'] * df['I3']
    ...
    df['KVAR'] = df.apply(lambda row: round(row['KVAR'], 2), axis=1)
...
if __name__ == "__main__":
    my_test_fun()  # added to ensure file does not auto run code if imported to other file without being called first
